...
namespace Tikshuv.FrontEnd.StorageWindow.StorageManager
{
    ...
    {
        private List<DataGridViewRow> deletedRows = new List<DataGridViewRow>();
        ...
        private void DgvTable_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            ...
            {
                // Cancel the deletion if user says no
                e.Cancel = true;
            }
            else
            {
                deletedRows.Add(e.Row);
                if(e.Row.Cells[5].Value?.ToString() !="N/A")
                {
                    MessageBox.Show("פריט לא יכול להימחק כיוון שהוא משויך למשימה");
                    e.Cancel = true;
                    return;
                }
                sendToServerDeletedItem();
            }
        }
        public List<DataGridViewRow> GetDeletedRows()
        {
            return deletedRows;
        }

        private void sendToServerDeletedItem()
        {
            string? type = deletedRows[0].Cells[0].FormattedValue?.ToString();
            string? serialNumber = deletedRows[0].Cells[2].FormattedValue?.ToString();
            Equipment deletedItem = new Equipment();
            deletedItem = Equipment.getSpecificEquipmentBySN(type,serialNumber);
            ConnectionHandler.SendRequest("equipment", "deleteItem", deletedItem);
        }
